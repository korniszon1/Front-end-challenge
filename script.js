...
async function getApi()
{
    ...
    console.log(source);    
    createAudio();
    setTimeout(function(){timeupdate()},900);
    ...
    for(i=0;i<list.length;i++)
    {
        if(i==0)
        {
            ...
            document.getElementById('space').innerHTML = '<div class="something"></div>';
            document.getElementById('space').innerHTML += '<div class="col-12 d-flex justify-content-between songlist"><img class="img-fluid col-3 col-lg-1 img-info" height="75px" src="'+img+'" onclick="changeSong('+i+')"><a onclick="changeSong('+i+')">'+(i+1)+''+curlist+'</a><div class="check"></div></div>';
        }else
        {
            img = list[i]['im:image'][2].label;
            const curlist= list[i].title.label;
